...
library(forcats)
coffee$country_of_origin <- fct_lump_min(coffee$country_of_origin, min = 10, other_level = "Other")

# Check the relationship between country and quality
table(coffee$country_of_origin, coffee$Qualityclass)
```
...
# AIC = 367.33, BIC = 384.77
model <- glm(Qualityclass ~ aroma + flavor + acidity, data = coffee,
...
#| echo: true
# Fit the model2
# AIC = 366.02, BIC = 387.82
model2 <- glm(Qualityclass ~ aroma + flavor + acidity + category_two_defects + altitude_mean_meters + harvested, data = coffee,
             family = binomial(link = "logit"))

# Stepwise regression using step()
stepwise_glm <- step(model2, direction = "both", trace = 1)
...
# AIC = 347.90, BIC = 369.70 

# coffee=coffee[,-ncol(coffee)]
